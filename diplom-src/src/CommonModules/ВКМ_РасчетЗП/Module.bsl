...
Функция РассчитатьОсновныеНачисления(Регистратор) Экспорт 
	...
	Запрос.Текст =                                              
		...
		|	СУММА(ЕСТЬNULL(ВКМ_ВыполненныеСотрудникомРаботыОбороты.ЧасовКОплатеПриход, 0)) КАК СуммаЧасовВыполненныхРабот,
		|	СУММА(ЕСТЬNULL(ВКМ_ВыполненныеСотрудникомРаботыОбороты.СуммаКОплатеПриход, 0)) КАК СуммаЗаВыполненныеРаботы
		|ИЗ
		...
		|	ЕСТЬNULL(ВКМ_ОсновныеНачисленияДанныеГрафика.РабочихДнейБазовыйПериод, 0),
